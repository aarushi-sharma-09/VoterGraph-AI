...
function LoginScreen({ onLogin }: { onLogin: (role: string, email: string) => void }) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`/api/auth/verify-otp`, { userId: otpUserId, otp });
      localStorage.setItem("token", res.data.token);
      onLogin(res.data.user.role, res.data.user.email);
    } catch (err: any) {
  ...
  const handleResendOtp = async () => {
    ...
    setOtpSuccess("");
    try {
      await axios.post(`/api/auth/resend-otp`, { userId: otpUserId });
...
function ChatScreen({ userRole, userEmail, onAdmin, onLogout }: { userRole: string; userEmail: string; onAdmin: () => void; onLogout: () => void }) {
  ...
  useEffect(() => {
    axios.get(`/api/geo/states`).then(res => setStates(res.data.states || [])).catch(console.error);
  }, []);
  ...
  useEffect(() => {
    ...
    if (selectedState) {
      axios.get(`/api/geo/districts?stateId=${selectedState}`).then(res => setDistricts(res.data.districts || [])).catch(console.error);
    }
  }, [selectedState]);
  ...
  useEffect(() => {
    ...
    setSelectedStation(""); setPollingStations([]);
    if (selectedDistrict) {
      axios.get(`/api/geo/constituencies?districtId=${selectedDistrict}`).then(res => setConstituencies(res.data.constituencies || [])).catch(console.error);
    }
  }, [selectedDistrict]);
  ...
  useEffect(() => {
    ...
    if (selectedConstituency) {
      axios.get(`/api/geo/polling-stations?constituencyId=${selectedConstituency}`).then(res => setPollingStations(res.data.pollingStations || [])).catch(console.error);
    }
...
useEffect(() => {
  ...
  const fetchSessions = async () => {
    try {
       const res = await axios.get(`/api/chat/sessions`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  const loadSession = async (sessionId: string) => {
    try {
       const res = await axios.get(`/api/chat/sessions/${sessionId}`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  const loadEarlier = async () => {
    ...
    try {
       const res = await axios.get(`/api/chat/sessions/${currentSessionId}?cursor=${nextCursor}`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
       const mapped = res.data.session.messages.map((m: any) => ({
         role: m.role,
         text: m.content,
         cypher: m.cypherQuery || m.cypher_query,
